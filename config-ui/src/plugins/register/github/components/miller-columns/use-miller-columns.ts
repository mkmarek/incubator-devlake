...
export const useMillerColumns = ({ connectionId }: UseMillerColumnsProps) => {
  ...
  const formatInstallations = (installations: any, parentId: McsID | null = null) =>
    installations.map((it: any) => ({
      parentId,
      id: it.id,
      title: it.account.login,
      type: 'org',
    }));
  ...
  useEffect(() => {
    (async () => {
      ...
      if (connection.authMethod === 'githubapp') {
        const appInstallations = await API.getAppInstallations(prefix);

        setUser(null);
        setLoaded(true, 'root', 2);
        setItems(formatInstallations(appInstallations));
      } else {
  ...
  const onExpand = useCallback(
    async (id: McsID) => {
      ...
      if (user && id === user.login) {
        ...
        const response = await API.getInstallationRepos(`plugins/github/connections/${connectionId}/installations/${id}/proxy/rest`, {
          page: 1,
          per_page: DEFAULT_PAGE_SIZE,
        });
